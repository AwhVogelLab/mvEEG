...
class RSA:
    def __init__(self,
        ...
        for factor in factors: # compute RDMs
            for i, condi in enumerate(condition_details):
                for j, condj in enumerate(condition_details):
                    self.theoretical_models[factor][i, j] = np.abs(condition_details[condi][factor] - condition_details[condj][factor])
        ...
        if per_subject_factors is not None:
            ...
            self.per_subject_models = defaultdict(lambda:np.full((per_subject_factors.shape[0],ncond,ncond),np.nan)) # subs x conds x conds
            for factor in per_subject_factors.keys():
                for i in range(ncond):
                    for j in range(ncond):
                        self.per_subject_models[factor][:,i,j] = np.abs(per_subject_factors[factor][:,i] - per_subject_factors[factor][:,j])
        ...
        self.dataset = DataLoader(root_dir=root_dir,
                                  data_type="rdms",
        ...
        if self.per_subject_models is not None:
            ...
            self.color_palette = {factor: sns.color_palette()[i] for i, factor in enumerate(
                list(self.theoretical_models.keys()) + list(self.per_subject_models.keys()))}
        else:
        ...
        for isub in range(self.nsub):
            ...
            if self.per_subject_models is None:
                ...
                factor_df = pd.DataFrame(np.transpose(
                    ...
                    [self.per_subject_models[model][isub, self.ridx, self.cidx] for model in self.per_subject_models.keys()]),
                    columns= models + list(self.per_subject_models.keys()))  # combination of RDMs and per subject models
    ...
    def plot_corrs(self, fac_order=None, y_sig=0.3, t_start=None, t_end=None,star_size=20,
        ...
        ax.hlines(0, xmin=-.5, xmax=3.5, color='black',
                   linestyle='--')  # 0 line
        ax = sns.barplot(data=delay_summary_df, x='factor', y='semipartial correlation',hue='factor',legend=False,
    ...
    def plot_corrs_temporal(self, title='Model Fits across time', stim_time=[0, 200], hide_stim=False, ax=None,
